...
import { Button, Table, Form, Input, notification, Row, Col, Icon, Modal } from 'antd';
...
class ParameterForm extends Component {
  ...
  renderColumn = ({ field, value, index }) => {
    ...
    return (
      <FormItem key={filedFormKey}>{this.renderInputWithType({ field, key, index })}</FormItem>
    );
  };
  ...
  renderInputWithType = ({ field, index, key, value }) => {
    ...
    const fieldProps = {
      rules: props.unique
        ? this.uniqueValidate(validatorProps)
        : [
            {
  ...
  renderEditable = () => {
    ...
    const formItems = !isEmpty(formKey) ? (
      map(formKey, (key, index) => (
      ...
      <Empty text={emptyText} inline={false} />
    );
  ...
  render() {
    ...
    const tableColumns = map(columns, field => {
      ...
      const columnsProps =
        isEmpty(compact(dataSource)) || !editable
          ? null
          : {
