...
const MODAL_FULL_HEIGHT =
  (document.documentElement.clientHeight || document.body.clientHeight) * 0.76;
...
let URL_PRE='UCT'
export const changeUrl=(pathname='uctmng/uct.htm/eff')=>{
  const pathArr=pathname.split('/');
 console.log(pathname.split('/'),'pathname.split()') 
 const headerMenu=['t3','base'];
 const index=headerMenu.some(item=>item===pathArr[2])?3:2;
 URL_PRE=pathArr.splice(0,index).join('/')
}
